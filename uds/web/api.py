...
def decode_uds_frame(msg: can.Message) -> Dict[str, Any]:
    ...
    if len(data) > 1:
        ...
        if frame_type == 0: sid = data[1]
        elif frame_type == 1: sid = data[2]
...
async def read_vin():
    ...
    try:
        with UDSClient() as client:
            ...
            return {"vin": resp.data.decode('ascii') if resp.data else "N/A", "status": resp.code_name}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def memory_dump():
    ...
    try:
        with UDSClient() as client:
            loc = udsoncan.MemoryLocation(address=0x08000000, memorysize=8, address_format=32, memorysize_format=8)
